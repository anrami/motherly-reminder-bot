...
import {Reminder, Resume} from "./types"
  ...
  async GetReminder(
    ...
    try {
      const raw = await this.runtime.callFunction(
        "GetReminder",
        {
          "momPrompt": momPrompt,"todo": todo
        },
        this.ctx_manager.cloneContext(),
        __baml_options__?.tb?.__tb(),
        __baml_options__?.clientRegistry,
      )
      return raw.parsed() as Reminder
    } catch (error: any) {
      const bamlError = createBamlValidationError(error);
      if (bamlError instanceof BamlValidationError) {
        throw bamlError;
...
class BamlStreamClient {
  ...
  GetReminder(
      momPrompt: string,todo: string,
      __baml_options__?: { tb?: TypeBuilder, clientRegistry?: ClientRegistry }
  ): BamlStream<RecursivePartialNull<Reminder>, Reminder> {
    try {
      const raw = this.runtime.streamFunction(
        "GetReminder",
        {
      ...
      return new BamlStream<RecursivePartialNull<Reminder>, Reminder>(
        raw,
        (a): a is RecursivePartialNull<Reminder> => a,
        (a): a is Reminder => a,
        this.ctx_manager.cloneContext(),
        __baml_options__?.tb?.__tb(),
      )
